...
- `internal/version`: build metadata variables (`Version`, `Commit`, `Date`) set via `ldflags` at build time
...
The `trello version` command returns build info embedded via `-ldflags -X`:
```
...
**Note on `AuthMode` serialization:** Auth response structs use `*string` for `AuthMode` so that `auth clear` can marshal `"authMode": null` per the command spec (Go's `omitempty` or `*string` with nil produces JSON `null`).
...
**`auth set` behavior:** `auth set` stores credentials without validating them against the Trello API. It does NOT call `GET /members/me` and does NOT return a `member` field in its response. This is intentional: `auth set` exists for headless/CI environments where the Trello API may not be reachable during credential setup. Use `auth status` to verify credentials after setting them.
...
**`--verbose` diagnostics:** When enabled, the API client logs to stderr via `slog`: HTTP method, URL (with credentials redacted), response status code, and request duration. Never log request/response bodies, tokens, or API keys.
...
- `set` stores credentials without calling Trello API (no `GET /members/me`)
- `set` response includes `configured: true`, `authMode: "manual"`, no `member` field
- `status` returns correct shape for configured vs unconfigured states
- `clear` removes credentials and returns `configured: false`, `authMode: null`
- `clear` response marshals `authMode` as JSON `null` (not empty string)
...
**Search result types** wrap the response to match the command spec's shape:
```go
...
```
These flow directly into the success envelope, producing `{"ok": true, "data": {"query": "...", "cards": [...]}}`.
